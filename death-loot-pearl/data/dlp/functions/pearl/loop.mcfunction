...
execute if entity @p[distance=..3] store success score @s dlp.match run data modify entity @s Item.tag.temp set from entity @p[distance=..3] UUID
execute if score #world dlp.cf.owner matches 0 if entity @p[distance=..3] run scoreboard players set @s dlp.match 0
